...
data = pd.read_csv("C:\\Users\\wes32\\lottery\\megamillions.csv", parse_dates=['date'], date_format='%m-%d-%y')
...
# number's probability. We can do this by looping through each number 1-75,
...
# through to the new normalized probability. I'm not too sure if this solution will
# actually calculate the correct probabilities. This was my best attempt


def normalizeNumber(probability):
    maximum = 0
    minimum = 10
    ...
    normalized = (probability - lowProb)/(highProb - lowProb)
    
    return normalized

for number, probability in sorted_probabilities:
    probability = normalizeNumber(probability)
